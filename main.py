...
def main():
    ...
    if (args.load is not None or (args.plot and not args.train)) and not os.path.isdir(
        utils.get_path(args)
    ):
        logging.error(
    ...
    if args.demo and not args.file:
        logging.error("Cannot run `demo` without a file. Try again with `--file FILE`.")
        exit(1)
    ...
    if (
        args.save
        and not args.init
        and not (args.train or args.test or args.demo or args.plot)
    ):
    ...
    if args.train or args.test:
        ...
        trainset, validset, testset = list(DataLoader.splits(samples, split_ratio))
        if args.sample_size is not None:  # limit samples used in training
            trainset = trainset[: args.sample_size]
            validset = validset[
                : int(args.sample_size * split_ratio[1] / split_ratio[0])
            ]
    ...
    if args.demo:
        if os.path.isfile(args.file):
            ...
            data = pd.DataFrame({"text": samples, "label": [0.5] * len(samples)})
            # Preprocess samples
            ...
            logging.error("Could not find file for demo at {}".format(args.file))
            exit(1)
    ...
    if args.test:
        if args.train or args.load is not None:
            ...
            logging.info("Testing accuracy: {:.4%}, loss: {:.6f}".format(acc, loss))
        else:
    ...
    if args.demo:
        if args.train or args.load is not None:
            ...
            for i, (text, _) in enumerate(data_loader):
                ...
                label = "{}{}{}".format(
                    "\033[92m" if pred.item() else "\033[93m", label, "\033[0m"
                )
