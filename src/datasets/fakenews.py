...
class FakeNews(Dataset):
    ...
    def load(self, download=True):
        ...
        if self.data is None:
            ...
            args = {"file": "train.csv", "url": self.trainurl if download else None}
            data = self.load_raw_file(**args)
            # Download test, submit files
            args = {"file": "test.csv", "url": self.testurl if download else None}
            self.load_raw_file(**args)
            args = {"file": "submit.csv", "url": self.submiturl if download else None}
            self.load_raw_file(**args)
